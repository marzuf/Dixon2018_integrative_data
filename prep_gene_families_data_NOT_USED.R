...
# => in the initial version, I filter based on the following tables:
...
# derived from pipeline consensus data
# here I want to have sth indpt from the TAD list !!!
# SO INSTEAD OF THE FOLLOWING TABLE
...
cat(paste0("> Rscript prep_gene_families_data.R\n"))
...
outFold <- paste0("../TAD_DE_pipeline_v2_", caller, "/", "PREP_GENE_FAMILIES_DATA")
dir.creates(outFold, recursive=TRUE)
...
outFile <- file.path(outFold, "hgnc_entrezID_family_DT.Rdata")
save(hgnc_entrezID_family_TAD_DT, file = outFile)
cat(paste0("... written: ", outFile, "\n"))

ensembl_entrezID_family_TAD_DT <- ensemblDT
outFile <- file.path(outFold, "ensembl_entrezID_family_DT.Rdata")
