createHTCfromMatrixObject <- function(chr.matrix, bin.size, chr, dim = -1){
  cat("...... start createHTCfromMatrixObject \n")
  options("scipen"=100, "digits"=6)
  
  #chr.matrix <- read.delim(file, header=F)
  ...
  chr.matrix = forceSymmetric(as.matrix(chr.matrix))
  ...
  write.table(as.matrix(chr.matrix), file ="tmp_check_matrix.txt", col.names=F, row.names=F, quote=F, sep="\t")
  #stop(" -- ok -- \n")
