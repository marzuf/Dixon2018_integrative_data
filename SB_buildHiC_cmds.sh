...
logfile="SB_buildHiC_logfile.txt"
rm -f $logfile
echo "--------------------------------------------------------------------------------------" >> ${logfile}
start_time=$(date -R)   
echo $start_time 
...
all_runs=( "SRR4272031" "SRR4272032" "SRR4272033" "SRR4272034" )
all_nbr=( "1" "2")

for run in ${all_runs[@]}; do   
    for nbr in ${all_nbr[@]}; do


    echo "*** start $run - $nbr" 
    echo "*** start $run - $nbr" >> $logfile
...
#    -U mate_R1.fastq.gz ) 2>>mate_R1.log | samtools view -Shb - > mate_R1.bam
    ...
    echo "$bwaBin mem -A1 -B4  -E50 -L0  $genomeFile ${run}_${nbr}.fastq | samtools view -Shb - > ${run}_${nbr}.bam"
    $bwaBin mem -A1 -B4  -E50 -L0  $genomeFile ${run}_${nbr}.fastq | samtools view -Shb - > ${run}_${nbr}.bam
    done
