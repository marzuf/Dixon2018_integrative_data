...
# Rscript prep_gene_families_TAD_data_otherTADfile.R GSE105318_ENCFF439QFU_DLD1
...
gene2tadDT_file <- file.path("gene_data_final", curr_ds, "genes2tad", "all_genes_positions.txt")    
...
TADpos_file <- file.path("gene_data_final", curr_ds, "genes2tad", "all_assigned_regions.txt")    
tadDT <- read.delim(TADpos_file, header=F, sep="\t", col.names = c("chromo", "region", "start", "end"), stringsAsFactors = F)
